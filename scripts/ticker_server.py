...
//   r|read|marks           a display read, trimmed to {vis} letters, up to {lanes} of them
...
def dump_box_dataset(config: TickerConfig, n_strands: int, path: Path) -> tuple[Path, int, int]:
    ...
    while len(records) < n_strands:
        ...
        for i, cluster in enumerate(clusters):
            ...
            final = final or ""
            marks, replaced, total = fix_marks(before or final, final, config.visible_letters)
            lines = [f"i|{len(records)}|{int(final == reference)}|{len(reads)}|{total}",
                     f"c|{final[:config.visible_letters]}|{marks}|{replaced}"]
            for r in subsample(reads, BASE_MAX_READS)[: config.visible_reads]:
                lines.append(f"r|{r[:config.visible_letters]}|"
                             f"{read_marks(r, final, config.visible_letters)}")
    ...
    body = BOX_HEADER_NOTE.format(channel=config.channel, reads=config.reads_per_strand,
                                  vis=config.visible_letters, lanes=config.visible_reads)
