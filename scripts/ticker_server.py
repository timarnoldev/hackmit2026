...
class DecodeBackend:
    ...
    def decode_one(self, cluster: Cluster, strand_length: int) -> tuple[str | None, str | None]:
        """(what came before, the final strand). Both None for an empty cluster.

        With the model live, "before" is the classic majority vote answer, so the difference
        is exactly what the model contributed.

        Without it, "before" is the medoid read, the single best raw read of the cluster.
        The difference is then exactly what the vote itself resolved, which is a real
        correction and the same picture as slide 3 of the deck. Either way the two strings
        are real decoder output and the diff between them is a real edit, never a guess.
        """
        if not any(cluster):
            return None, None
        final = _classic_draft(cluster, strand_length)
        if final is None:
            return None, None
        if not self.has_model:
            return _medoid_read(cluster, strand_length), final
        polished = self._decoder.decode([cluster], strand_length)[0]
        return final, (polished if polished is not None else final)


def _medoid_read(cluster: Cluster, strand_length: int) -> str | None:
    """The single raw read the classic decoder starts from, before any voting."""
    from dnacodec.baseline import _pick_draft

    reads = subsample([r for r in cluster if r], MAX_READS)
    return _pick_draft(reads, strand_length) if reads else None
...
# ---------------------------------------------------------------- embedded box dataset


BOX_HEADER_NOTE = """// Generated by scripts/ticker_server.py --dump-box. Do not edit by hand.
//
// A real run of the pipeline, frozen so the box can stand on a table with nothing but a USB
// power supply. Every strand here came out of dnacodec.encoder, went through
// dnacodec.simulator on the {channel} profile at {reads:g} reads per strand, and was decoded
// by the classic majority vote, the same decoder the box itself runs. Nothing is synthetic
// and nothing is hand written.
//
// Per strand the record carries the full reads and the true reference, which is what the
// on-device decoder needs, plus the display fields the Mac already computed, so the box can
// play the run even before it decodes anything itself.
//
// Record lines:
//   i|index|ok|nreads|nfix ok is 1 when the decoded strand matched the reference exactly,
//                          nfix counts the vote's corrections over the WHOLE strand, not
//                          just the letters the panel shows
//   c|consensus|fix|fixfrom   the first {vis} letters, the vote's corrections, and the
//                             letters the medoid read had where the vote changed them
//   r|read|marks           a display read, trimmed to {vis} letters, up to {lanes} of
//                          them. The box decodes for itself, so these are a cross check and
//                          a replay fallback, not what it draws.
//   f|read                 a full read, every one of them, for decoding on the device
//   x|reference            the true strand, only ever used to judge correctness
"""


def dump_box_dataset(config: TickerConfig, n_strands: int, path: Path) -> tuple[Path, int, int]:
    """Freeze a real run into a C++ header the firmware can compile in.

    Uses the classic majority vote on purpose, because that is what the box will run on its
    own. The header therefore never claims a result the device could not produce itself.
    """
    from dnacodec.baseline import MAX_READS as BASE_MAX_READS

    config = dc_replace(config).clamp()
    backend = DecodeBackend("/nonexistent-on-purpose.pt")  # the classic decoder, like the box
    records: list[str] = []
    seed_index = 0
    while len(records) < n_strands:
        data, _label = load_source(config.source, config.max_bytes)
        settings = EncoderSettings(strand_length=config.strand_length, redundancy=config.redundancy)
        profile = dc_replace(load_profile(config.channel), coverage_mean=config.reads_per_strand)
        encoded = encode(data, settings, None)
        clusters = simulate(encoded.strands, profile, train_seed(seed_index))
        seed_index += 1
        for i, cluster in enumerate(clusters):
            if len(records) >= n_strands:
                break
            reads = [r for r in cluster if r]
            reference = encoded.strands[i]
            if not reads:
                records.append(f"i|{len(records)}|0|0|0\nx|{reference}")
                continue
            before, final = backend.decode_one(cluster, config.strand_length)
            final = final or ""
            # the box's window, not the browser's: this file is only ever read by the box
            marks, replaced, total = fix_marks(before or final, final, BOX_LETTERS)
            lines = [f"i|{len(records)}|{int(final == reference)}|{len(reads)}|{total}",
                     f"c|{final[:BOX_LETTERS]}|{marks}|{replaced}"]
            for r in subsample(reads, BASE_MAX_READS)[:BOX_READS]:
                lines.append(f"r|{r[:BOX_LETTERS]}|{read_marks(r, final, BOX_LETTERS)}")
            for r in subsample(reads, BASE_MAX_READS):
                lines.append(f"f|{r}")
            lines.append(f"x|{reference}")
            records.append("\n".join(lines))

    path = Path(path)
    path.parent.mkdir(parents=True, exist_ok=True)
    body = BOX_HEADER_NOTE.format(channel=config.channel, reads=config.reads_per_strand,
                                  vis=BOX_LETTERS, lanes=BOX_READS)
    out = [body, "", "#pragma once", "", f"#define BOX_DATA_STRANDS {len(records)}",
           f"#define BOX_DATA_STRAND_LENGTH {config.strand_length}", "",
           "static const char *const kBoxData[] = {"]
    for rec in records:
        out.append('    "' + rec.replace("\n", "\\n") + '",')
    out.append("};")
    text = "\n".join(out) + "\n"
    path.write_text(text)
    return path, len(records), len(text)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--dump-box", default=None,
                   help="freeze a real run into a C++ header for the standalone box, then exit")
    p.add_argument("--box-strands", type=int, default=300, help="strands for --dump-box")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.dump_box:
        path, n, size = dump_box_dataset(config, args.box_strands, Path(args.dump_box))
        print(f"wrote {n} strands ({size / 1024:.0f} KB of source) to {path}")
        print("decoded with the classic majority vote, the decoder the box runs on its own")
        return 0
